...
- **Query ($Q$):** a learned projection that asks which context is relevant to a token.
- **Key ($K$):** a learned projection compared with each query to obtain compatibility scores.
- **Value ($V$):** the learned content vector combined with the normalized compatibility weights.
- **Scaling by $\sqrt{d_k}$:** controls dot-product magnitude, so the softmax does not saturate or yield extremely small (vanishing) gradients as $d_k$ grows.
- **Multi-head attention:** independently projects $Q$, $K$, and $V$ for each head, computes attention in parallel, then concatenates head outputs and applies $W^O$. This lets heads focus on different token relations and representation subspaces.
- **Encoder block:** multi-head self-attention and a position-wise feed-forward network are each wrapped by residual connections and layer normalization.
- **Positional encoding:** Model 1 adds fixed sinusoidal vectors, with $\operatorname{PE}(pos,2i)=\sin(pos/10000^{2i/d_{\text{model}}})$ and $\operatorname{PE}(pos,2i+1)=\cos(pos/10000^{2i/d_{\text{model}}})$, because attention alone does not encode token order.
- **Model 1 vs. Model 2:** Model 1 is a from-scratch Transformer encoder implementation whose parameters are initialized and trained on this project. Model 2 is BERT: a heavily pre-trained, bidirectional Transformer encoder that begins from learned weights, uses learned position embeddings, and sends the final `[CLS]` representation to the multiple-choice head.

*Basis: Vaswani et al. (2017) and Devlin et al. (2018).*
